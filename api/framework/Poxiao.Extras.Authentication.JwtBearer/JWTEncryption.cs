...
namespace Poxiao.DataEncryption;
...
public class JWTEncryption
{
    ...
    internal static JWTSettingsOptions SetDefaultJwtSettings(JWTSettingsOptions options)
    {
        ...
        {
            options.ValidAudience ??= "powerby Poxiao";
        }
    ...
    internal static Assembly GetFrameworkContext(Assembly callAssembly)
    {
        ...
        // 获取 Poxiao 程序集名称
        var poxiaoAssemblyName = callAssembly.GetReferencedAssemblies()
                                                   .FirstOrDefault(u => u.Name == "Poxiao")
                                                   ?? throw new InvalidOperationException("No `Poxiao` assembly installed in the current project was detected.");

        // 加载 Poxiao 程序集
        var poxiaoAssembly = AssemblyLoadContext.Default.LoadFromAssemblyName(poxiaoAssemblyName);

        // 获取 Poxiao.App 静态类
        FrameworkApp = poxiaoAssembly.GetType("Poxiao.App");

        return poxiaoAssembly;
    }
}