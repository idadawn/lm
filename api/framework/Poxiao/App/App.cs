...
public static class App
{
    ...
    private static (IEnumerable<Assembly> Assemblies, IEnumerable<Assembly> ExternalAssemblies) GetAssemblies()
    {
        ...
        if (!string.IsNullOrWhiteSpace(entryAssembly.Location))
        {
            ...
            // 读取项目程序集或 Poxiao 官方发布的包，或手动添加引用的dll，或配置特定的包前缀
            scanAssemblies = dependencyContext.RuntimeLibraries
               .Where(u =>
                      (u.Type == "project" && !excludeAssemblyNames.Any(j => u.Name.EndsWith(j))) ||
                      (u.Type == "package" && (u.Name.StartsWith(nameof(Poxiao)) || supportPackageNamePrefixs.Any(p => u.Name.StartsWith(p)))) ||
        ...
        else
        {
            ...
            if (singleFilePublishType != null)
            {
                ...
                // 解决 Poxiao.Extras.ObjectMapper.Mapster 程序集不能加载问题
                try
                {
            ...
            // 通过 AppDomain.CurrentDomain 扫描，默认为延迟加载，正常只能扫描到 Poxiao 和 入口程序集（启动层）
