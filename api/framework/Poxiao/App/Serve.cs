...
public static class Serve
{
    ...
    public static IHost RunNative(IRunOptions options, string urls = default)
    {
        ...
        IRunOptions runOptions = isSilence ? options : dynamicOptions.Silence(true, false);
    ...
    /// <returns><see cref="IHost"/></returns>
    public static IHost Run(
        Action<IServiceCollection> additional,
        string urls = default,
        bool silence = false,
        bool logging = false,
        string[] args = default
    ...
    public static IHost Run(IRunOptions options, string urls = default)
    {
        ...
        else if (options is GenericRunOptions genericRunOptions)
        {
            ...
            throw new InvalidCastException("Unsupported IRunOptions implementation type.");
    ...
    /// <param name="additional">配置额外服务</param>
    /// <returns><see cref="IHost"/></returns>
    public static IHost RunGeneric(
        bool silence = false,
        bool logging = false,
        string[] args = default,
        Action<IServiceCollection> additional = default
        ...
        where TStartup : class
    {
        ...
        builder = builder.ConfigureWebHostDefaults(webHostBuilder =>
        {
            ...
            var startUrls = !string.IsNullOrWhiteSpace(urls)
                ? urls
                : webHostBuilder.GetSetting(nameof(urls));
            ...
            {
                webHostBuilder = webHostBuilder.Configure(
                    (context, app) =>
                    {
                        // 注册应用中间件组件
            ...
            // 调用自定义配置
            webHostBuilder =
                options?.ActionWebDefaultsBuilder?.Invoke(webHostBuilder) ?? webHostBuilder;
        });
