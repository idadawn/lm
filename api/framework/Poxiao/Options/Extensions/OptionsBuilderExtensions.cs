...
public static class OptionsBuilderExtensions
{
    ...
    public static OptionsBuilder<TOptions> ConfigureBuilder<TOptions>(
    ...
    public static OptionsBuilder<TOptions> ConfigureBuilders<TOptions>(
        this OptionsBuilder<TOptions> optionsBuilder,
        IConfiguration configuration,
        ...
        where TOptions : class
    {
        ...
        // 获取所有构建器依赖接口
        var builderInterfaces = optionsBuilderType
            .GetInterfaces()
            .Where(u =>
                optionsBuilderDependency.IsAssignableFrom(u) && u != optionsBuilderDependency
            );
    ...
    /// <param name="configuration">配置对象</param>
    /// <returns>选项构建器实例</returns>
    public static OptionsBuilder<TOptions> ConfigureDefaults<TOptions>(
        this OptionsBuilder<TOptions> optionsBuilder,
        IConfiguration configuration
    )
        where TOptions : class
    {
        ...
        optionsBuilder.Bind(
            ...
            binderOptions =>
            {
                binderOptions.ErrorOnUnknownConfiguration =
                    optionsBuilderAttribute?.ErrorOnUnknownConfiguration ?? false;

                binderOptions.BindNonPublicProperties =
                    optionsBuilderAttribute?.BindNonPublicProperties ?? false;
            }
        );
        ...
        if (optionsBuilderAttribute?.ValidateOptionsTypes.IsEmpty() == false)
        {
            ...
            // 注册复杂选项
            Array.ForEach(
                optionsBuilderAttribute.ValidateOptionsTypes!,
                type =>
                {
                    optionsBuilder.Services.TryAddEnumerable(
                        ServiceDescriptor.Singleton(validateOptionsType, type)
                    );
                }
            );
        }
    ...
    private static void InvokeMapMethod(
    ...
    )
    {
        ...
        var optionsBuilderMethodMapAttribute =
            builderInterface.GetCustomAttribute<OptionsBuilderMethodMapAttribute>()!;
        ...
        var matchMethod = optionsBuilderType
            .GetMethods(bindingAttr)
            .First(u =>
                u.Name == methodName
                || u.Name.EndsWith("." + methodName)
                    && u.GetParameters().Length == genericArguments.Length
            );

        // 构建表达式实际传入参数
        var parameterExpressions = BuildExpressionCallParameters(
            matchMethod,
        ...
        var callExpression = Expression.Call(
            Expression.Constant(optionsBuilder),
            methodName,
    ...
    private static ParameterExpression[] BuildExpressionCallParameters(
    ...
    )
    {
        ...
        // 创建调用方法第一个委托参数表达式
        var delegateType = CreateDelegate(
            genericArguments,
            !isValidateMethod ? default : typeof(bool)
        );
    ...
    internal static Type CreateDelegate(Type[] inputTypes, Type outputType = default)
    {
        ...
        return !isFuncDelegate
            ? baseDelegateType
                .Assembly.GetType($"{baseDelegateType.FullName}`{inputTypes!.Length}")!
                .MakeGenericType(inputTypes)
            : baseDelegateType
                .Assembly.GetType(
                    $"{(baseDelegateType.FullName![0..^2])}`{inputTypes!.Length + 1}"
                )!
