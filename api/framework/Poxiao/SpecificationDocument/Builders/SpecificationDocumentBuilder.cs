...
public static class SpecificationDocumentBuilder
{
            ...
            if (File.Exists(assemblyXmlPath))
            {
                ...
                foreach (var memberElement in memberElementList)
                {
                    ...
                    if (value == null)
                    {
                        ...
                        // 处理隐式实现接口的注释
                        // 注释格式：M:Poxiao.Application.TestInheritdoc.Poxiao#Application#ITestInheritdoc#Abc(System.String)
                        // 匹配格式：[A-Z]:[a-zA-Z_@\.]+\.
                        ...
                        // 处理带参数的注释
                        // 注释格式：M:Poxiao.Application.TestInheritdoc.WithParams(System.String)
                        // 匹配格式：[A-Z]:[a-zA-Z_@\.]+
                        ...
                        // 处理不带参数的注释
                        // 注释格式：M:Poxiao.Application.TestInheritdoc.WithParams
                        // 匹配格式：无
    ...
    private static void CustomizeIndex(SwaggerUIOptions swaggerUIOptions)
    {
        ...
        // 判断是否启用 MiniProfile
        var customIndex = $"{Reflect.GetAssemblyName(thisAssembly)}{thisType.Namespace.Replace(nameof(Poxiao), string.Empty)}.Assets.{(_appSettings.InjectMiniProfiler != true ? "index" : "index-mini-profiler")}.html";
