...
public static partial class Extensions
{
    ...
    /// 根据时间描述信息获取时间范围.
    /// </summary>
    /// <param name="description">根据时间描述信息</param>
    /// <returns></returns>
    public static (DateTime start, DateTime end) GetTimeRange(this string description)
    {
        ...
        switch (description)
        {
            ...
            case "昨天":
                ...
                break;
            case "本周":
                start = now.AddDays(1 - Convert.ToInt32(now.DayOfWeek.ToString("d")));
                break;
            case "上周":
                start = now.AddDays(1 - Convert.ToInt32(now.DayOfWeek.ToString("d")) - 7);
                end = now.AddDays(0 - Convert.ToInt32(now.DayOfWeek.ToString("d")));
                break;
            case "本月":
                start = now.AddDays(1 - now.Day);
                break;
            case "上月":
                start = now.AddMonths(-1).AddDays(1 - now.AddMonths(-1).Day);
                end = now.AddDays(0 - now.Day);
                break;
            case "本年":
