...
public static class StringExtensions
{
    ...
    /// <summary>
    /// 以指定字符串作为分隔符将指定字符串分隔成List<string>.
    /// </summary>
    /// <param name="value">要分割的字符串.</param>
    /// <param name="strSplit">字符串类型的分隔符.</param>
    /// <param name="removeEmptyEntries">是否移除数据中元素为空字符串的项.</param>
    ...
    /// <returns>转换后的枚举值，如果转换失败则返回默认枚举值。</returns>
    ...
    /// 是否可聚合.
    /// </summary>
    public static bool IsCanAgg(this string value)
    {
        try
        {
            if (value == null || value.Trim().IsNullOrEmpty())
                return false;
            var flag = AggTypeConst.DbIntTypes.Exists(x => x.ToLower().Equals(value.ToLower()));
            return flag;
        }
        catch (Exception)
        {
    ...
    /// 是否指标值筛选方式.
    /// </summary>
    public static bool IsByValueFilterModel(this string value)
    {
        try
        {
            if (value.Trim().IsNullOrEmpty())
                return false;
            var flag = AggTypeConst.DbByValueFilterModel.Exists(x => x.ToLower().Equals(value.ToLower()));
            return flag;
        }
        catch (Exception)
        {
    ...
    /// 是否指标日期范围筛选方式.
    /// </summary>
    public static bool IsByDateRangFilterModel(this string value)
    {
        try
        {
            if (value.Trim().IsNullOrEmpty())
                return false;
            var flag = AggTypeConst.DbByDateRangeFilterModel.Exists(x => x.ToLower().Equals(value.ToLower()));
            return flag;
        }
        catch (Exception)
        {
    ...
    /// <returns></returns>
    public static string ReplaceMetricValue(this string? value, string oldValue, string newValue, string defaultValue = "0")
    {
        return ReplaceDefault(value, oldValue, newValue, defaultValue);
    }
    ...
    /// <param name="defaultValue"></param>
    /// <returns></returns>
    public static string ReplaceDefault(this string? value, string oldValue, string newValue, string defaultValue = "")
    {
        if (value == null)
            return string.Empty;

        return value.Contains(oldValue) ? value.Replace(oldValue, newValue) : defaultValue;
    }
