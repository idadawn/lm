extern alias MathNetOfficial;
using MathStatistics = MathNetOfficial::MathNet.Numerics.Statistics.Statistics;
...
public class AnalysisDataService : IAnalysisDataService, ITransient
{
    ...
    public AnalysisDataNormalListOutput GetNHChart()
    {
        ...
        var nmData = MathNetOfficial::MathNet.Numerics.Generate.Normal(dataNum, 0, 1); // 生成符合正态分布的数据
        var ds = new MathNetOfficial::MathNet.Numerics.Statistics.DescriptiveStatistics(nmData, true);
        ...
        var hg = new MathNetOfficial::MathNet.Numerics.Statistics.Histogram(nmData, colSum, minSpeed, maxSpeed);
    ...
    public AnalysisXbarRbarOutput GetXRChart()
    {
        ...
        {
            xrOut.Average.Axis.Add(Math.Round(MathStatistics.Mean(item), 4));
            xrOut.Range.Axis.Add(Math.Round(MathStatistics.Maximum(item) - MathStatistics.Minimum(item), 4));
        }

        // 设置均值图管制限
        var acl = MathStatistics.Mean(xrOut.Average.Axis);
        var a2R = _factorChart["A2"][_xRProtoData[0].Count - 2] * MathStatistics.Mean(xrOut.Range.Axis);
        xrOut.Average.UCL = Math.Round(acl + a2R, 5);
        xrOut.Average.LCL = Math.Round(acl - a2R, 5);
        xrOut.Average.CL = Math.Round(acl, 5);

        // 设置极差图管制限
        var rcl = MathStatistics.Mean(xrOut.Range.Axis);
