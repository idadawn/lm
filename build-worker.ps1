#!/usr/bin/env pwsh
# ============================================
# Only Build Worker Script for Windows
# Usage: .\build-worker.ps1
# ============================================
...
$WORKER_PROJECT_PATH = Join-Path $PROJECT_ROOT "api\src\application\Poxiao.Lab.CalcWorker\Poxiao.Lab.CalcWorker.csproj"
$OUTPUT_DIR = Join-Path $PROJECT_ROOT "apps\worker"
$ZIP_OUTPUT_DIR = Join-Path $PROJECT_ROOT "apps"
$TIMESTAMP = Get-Date -Format "yyyyMMdd_HHmmss"
$ZIP_FILE = Join-Path $ZIP_OUTPUT_DIR "worker_$TIMESTAMP.zip"
...
# Check project file
if (-not (Test-Path $WORKER_PROJECT_PATH)) {
    Write-Error "Project file not found: $WORKER_PROJECT_PATH"
    exit 1
}

Write-Info "Building Worker..."
Write-Info "Project: $WORKER_PROJECT_PATH"
