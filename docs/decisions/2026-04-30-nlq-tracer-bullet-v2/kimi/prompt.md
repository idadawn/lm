# 你是 nlq-agent tracer-bullet 的 KIMI 工人

你正在 git worktree 中独立执行一个完整的 ralplan-approved 实现任务。
GLM 工人在并行执行同一份计划（不同 worktree）。我（lead）4 小时后会来对比验收两份实现。

## 工作区（硬性约束）
- 你的工作目录：**`/data/project/lm-team/kimi/`**（git 分支 `omc-team/nlq-tracer/kimi`）
- 你**只能**修改这个目录下的文件。
- 计划文档（只读，绝对路径）：**`/data/project/lm/.omc/plans/nlq-tracer-bullet-v2.md`**
- 不要触碰 `/data/project/lm/`（主仓库）或 `/data/project/lm-team/glm/`（GLM 工人的工作区）。
...
- 仓库根（在你的 worktree 内）：`/data/project/lm-team/kimi/nlq-agent/`
- 项目约定见 `/data/project/lm-team/kimi/CLAUDE.md` 和 `/data/project/lm-team/kimi/.cursorrules`。
...
- 完成后用 `git log --oneline omc-team/nlq-tracer/kimi` 自检。

## 输出
完成时：
1. `git status` 干净，`git log` 至少 3 条 conventional commits。
2. 在 `/data/project/lm/.omc/team-nlq/kimi/REPORT.md` 写一份验收报告：
...
## 失败时
如果遇到无法克服的阻塞，把详细诊断写到 `/data/project/lm/.omc/team-nlq/kimi/BLOCKER.md` 然后退出。
