...
- 任何 breaking change 都需要在 `services/agent-api/app/models/schemas.py` 中同步实施，并通过 `services/agent-api/tests` 中的协议序列化用例验证。
...
## Dependencies
### Internal
- 消费方：`nlq-agent/services/agent-api/app/models/schemas.py`、`web/src/types/reasoning-protocol.d.ts`、`mobile/utils/sse-client.js`
