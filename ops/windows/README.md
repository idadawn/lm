...
WPF 控制台按“部署商店”方式组织：下载基础软件、安装服务、一键更新、服务控制、数据库备份。
...
WPF 控制台使用 `.env` 中的授权码控制功能入口：

| 角色 | 配置项 | 权限 |
|------|--------|------|
| 管理人员 | `MANAGER_AUTH_CODE` | 发布完整版本 / 工具补丁到 OSS，并拥有维护人员权限 |
| 维护人员 | `MAINTAINER_AUTH_CODE` | 下载基础软件、安装/更新程序、启动/停止/重启服务、查询状态、数据库备份 |

- **首次启动**会自动弹出「身份授权」对话框；也可随时点左侧「输入授权码/切换授权」，或「退出授权」。
- `.env` 中授权码为空时，首次启动自动生成随机授权码写入 `ops\.env`，并在对话框和执行输出中显示，请妥善保存；正式部署请改成自己的授权码。
...
在线更新/发布所需的 OSS 凭据不必手工编辑 `.env`：控制台「更新」页有「配置 OSS 凭据码」入口，
点「检查并更新 / 安装每日自动更新任务 / 发布」时若未配置也会先提示输入。
注意与上面的**身份授权码**是两回事：身份授权码决定谁能操作控制台，OSS 凭据码提供更新/发布用的存储凭据。

- 凭据码格式：`Base64("LMAUTH1|endpoint|bucket|accessKeyId|accessKeySecret|prefix")`，
  保存后自动写入 `ops/.env` 的 `OSS_*` 键（`.env` 不存在时以 `.env.example` 为底渲染）。
- 生成方式：在已配置好 OSS 的机器（如构建机）上打开凭据码对话框，点「复制当前凭据码」，
  把这一串码发给现场粘贴即可。
