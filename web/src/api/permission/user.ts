...
export function getUserInfoList(userId) {
  const data = { userId };
  return defHttp.post({ url: Api.Prefix + `/getUserList`, data });
}
// 获取我的下属
...
// 获取选中组织、岗位、角色、用户基本信息
export function getSelectedList(userId) {
  return defHttp.post({ url: Api.Prefix + `/getSelectedList`, data: { userId } });
}
// 获取选中组织、岗位、角色、用户基本信息
