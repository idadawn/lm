...
export function exportData(id) {
  return defHttp.get({ url: Api.Prefix + `/${id}/Action/Export` });
}
// 获取数据接口调用日志列表
...
export function getDataInterfaceDataSelect(data) {
  return defHttp.post({ url: Api.Prefix + `/${data.interfaceId}/Action/List`, data });
}
// 获取多条接口详情数据
export function getDataInterfaceDataInfoByIds(id, data) {
  return defHttp.post({ url: Api.Prefix + `/${id}/Action/InfoByIds`, data });
}
// 获取接口列表(工作流选择时调用，带分页)
