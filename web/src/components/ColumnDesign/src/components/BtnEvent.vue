        ...
        <a-form :colon="false" labelAlign="left" :labelCol="{ style: { width: '90px' } }" :model="dataForm" :rules="formRules" ref="formElRef" hideRequiredMark>
          ...
          <template v-if="dataForm.btnType == 1">
            <a-form-item label="选择表单" name="modelId">
              <jnpf-tree-select v-model:value="dataForm.modelId" :options="treeData" placeholder="请选择表单" lastLevel allowClear @change="onModeIdChange" />
            </a-form-item>
...
<script lang="ts" setup>
  ...
  const defaultData = {
    ...
    formOptions: [],
    customBtn: false,
    func: '({ data, index, request, toast, refresh }) => {\r\n   \r\n}',
    interfaceId: '',
    interfaceName: '',
    templateJson: [],
    useConfirm: false,
  ...
  function init(data) {
    ...
    formFieldsOptions.value = data.formFieldsOptions
      .filter(o => !noAllowList.includes(o.__config__.jnpfKey) && o.id.indexOf('-') < 0)
      .map(o => ({ ...o, disabled: false }));
    getFeatureList();
    getFieldOptions();
  }
  function getFeatureList() {
    getVisualDevSelector({ type: 1, webType: '1,2', enableFlow: 0 }).then(res => {
