        ...
        <template #bodyCell="{ column, record, index }">
          ...
          <template v-else-if="column.isChild">
            <template v-for="item in state.columnsChildList">
              <template v-if="item.children && item.children[0] && column.dataIndex === item.children[0]?.dataIndex">
...
<script lang="ts" setup>
  ...
  function handleNext() {
    if (state.activeStep == 0) {
      ...
      getImportPreview(state.apiUrl, { fileName: state.fileName })
        .then(res => {
          ...
          state.resultColumnsChildList = state.resultColumns.filter(o => o.children && o.children.length);
          const preHeaderList = cloneDeep(headerList);
          for (let i = 0; i < preHeaderList.length; i++) {
            if (preHeaderList[i].children?.length) {
          ...
          state.columnsChildList = state.columns.filter(o => o.children && o.children.length);
          state.btnLoading = false;
          state.activeStep += 1;
        })
        .catch(() => {
    ...
    if (state.activeStep == 1) {
      ...
      state.btnLoading = true;
      importData(state.apiUrl, { list: state.list })
        .then(res => {
