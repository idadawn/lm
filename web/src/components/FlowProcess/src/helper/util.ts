...
export class NodeUtils {
  ...
  static checkNode(node, _parent?) {
    ...
    this.isSubFlowNode(node) &&
      (!props.flowId ||
        (props.initiateType === 6 &&
          isEmptyArray(props.initiator) &&
          isEmptyArray(props.initiatePos) &&
          isEmptyArray(props.initiateRole) &&
          isEmptyArray(props.initiateOrg) &&
          isEmptyArray(props.initiateGroup))) &&
      (valid = false);

    this.isApproverNode(node) &&
      (!props.assigneeType || props.assigneeType === 6) &&
      isEmptyArray(props.approvers) &&
      isEmptyArray(props.approverPos) &&
      isEmptyArray(props.approverRole) &&
      isEmptyArray(props.approverOrg) &&
      isEmptyArray(props.approverGroup) &&
      (valid = false);
    return valid;
  }
  /**
