<template>
  <div class="right-board common-board">
    ...
    <div
      class="field-box"
      v-if="activeData?.__config__ && activeData.__config__.jnpfKey && !['tableGridTr', 'tableGridTd'].includes(activeData.__config__.jnpfKey)">
      <ScrollContainer>
        <a-form :colon="false" labelAlign="left" :labelCol="{ style: { width: '90px' } }" v-show="activeKey === 'field'" class="right-board-form">
          ...
          <a-form-item label="控件用途" v-if="dynamicModelList.includes(jnpfKey)">
          ...
          <a-form-item label="卡片标题" v-if="jnpfKey === 'card'">
            <a-input v-model:value="activeData.header" placeholder="请输入卡片标题" />
          </a-form-item>
          <a-form-item label="标题提示" v-if="hasTipLabel(jnpfKey)">
            <jnpf-textarea v-model:value="activeData.__config__.tipLabel" placeholder="请输入标题提示" :rows="2" />
          </a-form-item>
          ...
          <a-form-item label="显示内容" v-if="jnpfKey === 'currOrganize'">
            <a-select v-model:value="activeData.showLevel" placeholder="请选择显示内容">
...
<script lang="ts" setup>
  ...
  import { dynamicModelList, layoutList, systemList, orgList } from './helper/rightPanel';
  ...
  function getDictionaryType() {
    getDictionaryTypeSelector().then(res => {
      state.dictionaryOptions = res.data.list;
    });
  }
