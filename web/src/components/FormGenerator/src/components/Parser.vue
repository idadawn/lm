<script lang="tsx">
  ...
  import { getScriptFunc, getDateTimeUnit, getTimeUnit } from '/@/utils/jnpf';
  ...
  export default defineComponent({
    ...
    setup(props, { expose, emit }) {
          ...
          if (config.jnpfKey === 'collapse') {
            return (
              <a-col span={config.span} class={props.formConf.formStyle ? '' : 'mb-20px'}>
                <a-collapse
                  ghost={element.ghost}
                  expandIconPosition={element.expandIconPosition}
        ...
        return (
          <a-row class={unref(getFormClass)}>
            <a-form
              ...
              colon={state.formConfCopy.colon}
      ...
      function request(url, method: string, data) {
        if (!url) return;
        const realMethod = method.toLowerCase() === 'post' ? 'post' : 'get';
        return defHttp[realMethod]({ url, data });
      }
        ...
        componentList.forEach(cur => {
          ...
          if (dyOptionsList.includes(config.jnpfKey)) {
            if (config.dataType === 'dictionary') {
              ...
              let query = {
                paramList: config.templateJson ? getParamList(config.templateJson) : [],
              };
              getDataInterfaceRes(config.propsUrl, query).then(res => {
