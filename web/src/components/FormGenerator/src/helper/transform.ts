...
export const getSearchFormSchemas = (list: any[]) => {
  ...
  for (let i = 0; i < schemas.length; i++) {
    ...
    if (['InputNumber', 'Calculate'].includes(e.component)) {
      e.component = 'NumberRange';
      e.componentProps = { precision: !e.componentProps.precision && e.component === 'Calculate' ? 0 : e.componentProps.precision };
    }
    ...
    if (['CurrOrganize', 'OrganizeSelect'].includes(e.component)) {
      e.component = 'OrganizeSelect';
      e.componentProps = { placeholder: '请选择' + e.label, multiple: e.componentProps.searchMultiple || false };
    }
    if (e.component === 'CurrPosition') {
      e.component = 'PosSelect';
      e.componentProps = { placeholder: '请选择' + e.label, multiple: e.componentProps.searchMultiple || false };
    }
    if (['GroupSelect', 'RoleSelect'].includes(e.component)) {
      e.componentProps = { placeholder: '请选择' + e.label, multiple: e.componentProps.searchMultiple || false };
    }
    if (e.component === 'UserSelect') {
      e.componentProps = {
        ...
        ableDepIds: e.componentProps.ableDepIds,
        ablePosIds: e.componentProps.ablePosIds,
        ableUserIds: e.componentProps.ableUserIds,
        ableRoleIds: e.componentProps.ableRoleIds,
        ableGroupIds: e.componentProps.ableGroupIds,
        multiple: e.componentProps.searchMultiple || false,
        placeholder: '请选择' + e.label,
      };
    }
