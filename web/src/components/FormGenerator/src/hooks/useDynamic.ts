...
import { noAllowRelationList } from '/@/components/FormGenerator/src/helper/config';

export function useDynamic(activeData) {
  ...
  const onPropsUrlChange = (val, row) => {
    ...
    const list = row.requestParameters ? JSON.parse(row.requestParameters) : [];
    ...
    const query = {
      paramList: activeData.__config__.templateJson || [],
    };
