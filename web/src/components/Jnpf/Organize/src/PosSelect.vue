...
<script lang="ts" setup>
  ...
  function handleSelect(keys) {
    ...
    if (data?.disabled || data?.type !== 'position') return;
  ...
  async function initData() {
    loading.value = true;
    allList.value = await organizeStore.getPositionList();
    if (props.selectType === 'all') {
      treeData.value = await organizeStore.getPositionTree();
      loading.value = false;
    } else {
      const query = {
        departIds: props.ableDepIds,
        positionIds: props.ablePosIds,
      };
