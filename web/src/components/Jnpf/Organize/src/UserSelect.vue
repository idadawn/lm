      ...
      <div class="transfer-pane left-pane">
        ...
        <div class="transfer-pane__body transfer-pane__body-tab" v-if="selectType === 'all'">
          ...
          <ScrollContainer v-loading="loading" v-else>
            ...
            <Empty :image="simpleImage" v-if="!ableList.length" />
          </ScrollContainer>
        </div>
...
<script lang="ts" setup>
  ...
  const ableQuery = reactive<Recordable>({
    departIds: [],
    positionIds: [],
    userIds: [],
    roleIds: [],
  ...
  function openSelectModal() {
    ...
    if (props.selectType === 'custom') {
      ableQuery.departIds = props.ableDepIds;
      ableQuery.positionIds = props.ablePosIds;
      ableQuery.userIds = props.ableUserIds;
      ableQuery.roleIds = props.ableRoleIds;
      ableQuery.groupIds = props.ableGroupIds;
    } else {
      const id = getAbleKey(props.selectType);
      ableQuery.departIds = [];
      ableQuery.positionIds = [];
      ableQuery.userIds = [];
      ableQuery.roleIds = [];
      ableQuery.groupIds = [];
      ableQuery[id] = Array.isArray(props.ableRelationIds) ? props.ableRelationIds : [props.ableRelationIds];
    }
    ableList.value = [];
    finish.value = false;
    getAbleList();
