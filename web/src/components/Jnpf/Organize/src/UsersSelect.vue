...
<script lang="ts" setup>
  ...
  function setValue() {
    ...
    getSelectedList(unref(ids)).then(res => {
      if (!props.value || !props.value.length) return setNullValue();
      const selectedList: any[] = res.data.list.map(o => ({ ...o, id: o.type ? o.id + '--' + o.type : o.id }));
  ...
  function handleNodeClick(data) {
    const usableList = props.multiple ? ['company', 'department', 'role', 'position', 'group', 'user'] : ['user'];
