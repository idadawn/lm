...
export function useMindMap({ props, graphRef, deleteItem, addItem }) {
  let { source, authAdd, authDelete } = toRaw(props);
  ...
  G6.registerNode(
    'node-with-line',
    {
      ...
      draw(cfg: CustomModelConfig, group) {
        ...
        const { id, name, currentValue, metricName, trendData, statusColor, metricGrade } = cfg;
        ...
        const childrenLen = source.edges.reduce((pre, cur) => {
          if (cur.source === id) {
            pre++;
          }
          return pre;
  ...
  const initGraph = () => {
    ...
    const _graph = new G6.Graph({
      ...
      modes: {
        default: ['drag-canvas', 'zoom-canvas', 'drag-node', 'collapse-expand-combo'],
      },
      defaultNode: {
        type: 'node-with-line',
        size: nodeConfig.size,
    ...
    graph = _graph;
    _graph.data(cloneSource);
    _graph.render();
    // _graph.fitView();
    // _graph.moveTo(10, 10, true, {
    //   duration: 100,
    // });
    ...
    _graph.on('node:click', async (evt: any) => {
      ...
      switch (name) {
        ...
        case 'collapse-text':
          ...
          // 点击节点的id
          const clickNodeId = model.id;
          // 找到需要隐藏的节点和边
          const task = nodeId => {
            const edges = _graph.getEdges();
            const run = id => {
              for (let i = 0; i < edges.length; i++) {
                const expendSource = edges[i].getSource().getModel();
                const expendTarget = edges[i].getTarget().getModel();
                if (expendSource.id === id) {
                  const expendNode = edges[i].getTarget();
                  if (collapsed) {
                    edges[i].show();
                    expendNode.show();
                  } else {
