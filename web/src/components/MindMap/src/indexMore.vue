<script setup lang="ts">
  ...
  import { useMindMap, useMindMapResult } from '../hooks/useMindMapMore';
   ...
   *@param{String}nodeId
   **/
  const deleteAssociationItem = (nodeId: string | number) => {
    const edges = graph.value.getEdges();
    const deleteItems: string[] = [];
    const run = id => {
      for (let i = 0; i < edges.length; i++) {
        const source = edges[i].getSource().getModel();
        const target = edges[i].getTarget().getModel();
        if (source.id === id) {
          run(target.id);
          deleteItems.push(target.id);
        }
      }
    };
    run(nodeId);
    deleteItems.forEach(res => {
      graph.value.removeItem(res);
    });
  };
