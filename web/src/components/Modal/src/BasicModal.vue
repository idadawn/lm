<template>
  <Modal v-bind="getBindValue" @cancel="handleCancel">
    ...
    <ModalWrapper :useWrapper="getProps.useWrapper" :footerOffset="wrapperFooterOffset" :fullScreen="fullScreenRef"
      ...
      :minHeight="getProps.minHeight" :height="getWrapperHeight" :visible="openRef"
      :modalFooterHeight="footer !== undefined && !footer ? 0 : undefined"
      v-bind="omit(getProps.wrapperProps, 'visible', 'open', 'height', 'modalFooterHeight')"
...
export default defineComponent({
  ...
  inheritAttrs: false,
  props: basicProps,
  emits: ['open-change', 'height-change', 'cancel', 'ok', 'register', 'update:open', 'update:visible'],
  setup(props, { emit, attrs, slots }) {
    ...
    watchEffect(() => {
      // 支持 open 和 visible 两种属性，优先使用 visible
      openRef.value = !!(props.visible !== undefined ? props.visible : props.open);
    ...
    function setModalProps(props: Partial<ModalProps>): void {
      ...
      // 支持 visible 和 open 两种属性，优先使用 visible
      if (Reflect.has(props, 'visible')) {
        openRef.value = !!props.visible;
      } else if (Reflect.has(props, 'open')) {
