<template>
  <div :class="[bem(), 'h-full']">
    <TreeHeader :title="title" :helpMessage="helpMessage" :showToolbar="showToolbar" :dropDownActions="dropDownActions"
      :expandAll="expandAll" :checkAll="checkAll" @strictly-change="onStrictlyChange" @reload="reload"
      @clearSearch="searchValue = ''" :isAsync="!!loadData" v-if="title || dropDownActions.length">
      <slot name="headerTitle" v-if="slots.headerTitle"></slot>
    </TreeHeader>
    <div :class="bem('search')" v-if="showSearch">
      <InputSearch :placeholder="searchPlaceholder || t('common.leftTreeSearchText')" allowClear
        v-model:value="searchValue" />
    </div>
    <BasicTree :clickRowToExpand="false" :treeData="treeData" :fieldNames="fieldNames" :checkStrictly="checkStrictly"
      :load-data="loadData" @select="handleSelect" class="tree-main" ref="treeRef" :defaultExpandAll="defaultExpandAll"
      :selectedKeys="selectKeysId" :loading="loading">
...
import type { FieldNames, DropDownActionItem, TreeActionType, KeyType, selectKeysId } from './types/tree';
import { BasicTree } from '/@/components/Tree';
import TreeHeader from './components/LeftTreeHeader.vue';
...
const props = defineProps({
  ...
  selectKeysId: {
    type: Array as PropType<selectKeysId[]>,
    default: () => [],
  },
  loadData: {
    type: Function,
...
// const selectKeysId = ref(['base_account']);
