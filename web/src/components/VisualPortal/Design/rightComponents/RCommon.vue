  ...
  <a-collapse-panel key="1">
    ...
    <template v-if="activeData.jnpfKey == 'timeAxis'">
      <a-form-item>
        ...
        <jnpf-select v-model:value="activeData.option.layout" :options="layoutStyleList" placeholder="请选择风格类型" showSearch />
      </a-form-item>
      <a-form-item label="排序">
        <jnpf-radio v-model:value="activeData.option.sortable" :options="sortList" optionType="button" buttonStyle="solid" />
      </a-form-item>
      <a-form-item>
        <template #label>显示条数<BasicHelp text="置空时，则显示全部数据" /></template>
        <a-input-number v-model:value="activeData.option.showNumber" min="0" :max="500" v-if="showType == 'pc'" />
        <a-input-number v-model:value="activeData.option.appShowNumber" min="0" :max="500" v-else />
      </a-form-item>
    </template>
    <template v-if="activeData.jnpfKey == 'text'">
      ...
      <a-form-item label="文本内容" v-if="activeData.dataType == 'static'">
        <a-input v-if="activeData.option.styleType == 1" v-model:value="activeData.option.defaultValue" placeholder="请输入文本内容" />
    ...
    <template v-if="activeData.jnpfKey == 'image'">
      ...
      <a-form-item v-else-if="activeData.option.styleType == 2">
        ...
        <a-input v-model:value="activeData.option.defaultValue" placeholder="请输入图片地址" />
      </a-form-item>
      <a-form-item label="数据接口" v-else>
        <interface-modal :value="activeData.propsApi" :title="activeData.propsName" popupTitle="数据接口" @change="onPropsApiChange" />
      </a-form-item>
      ...
      <a-form-item label="填充方式">
        <jnpf-select v-model:value="activeData.option.imageFillStyle" :options="fillStyleList" placeholder="请选择填充方式" showSearch />
      </a-form-item>
      <a-form-item label="文本内容">
        <a-input v-model:value="activeData.option.textDefaultValue" placeholder="请输入文本内容" />
      </a-form-item>
    ...
    <template v-if="activeData.jnpfKey == 'carousel'">
      ...
      <a-form-item label="轮播间隔">
        <a-input-number v-model:value="activeData.option.carouselInterval" placeholder="请输入轮播间隔" :min="500" :max="500000" addon-after="ms" />
      </a-form-item>
    ...
    <template v-if="activeData.jnpfKey == 'video'">
      ...
      <a-form-item v-else-if="activeData.option.styleType == 2">
        ...
        <a-input v-model:value="activeData.option.defaultValue" placeholder="请输入视频地址" />
      </a-form-item>
      <template v-else>
    ...
    <template v-if="activeData.jnpfKey == 'iframe'">
      <a-form-item>
        ...
        <a-input v-model:value="activeData.option.defaultValue" placeholder="请输入链接地址" />
      </a-form-item>
    </template>
    <template v-if="activeData.jnpfKey == 'todo'">
      <template v-if="showType == 'app'">
        <a-form-item label="风格类型">
          <jnpf-select v-model:value="activeData.option.styleType" :options="appTodoStyleList" placeholder="请选择风格类型" showSearch />
        </a-form-item>
        <template v-if="activeData.option.styleType == 2">
    ...
    <template v-if="activeData.jnpfKey == 'commonFunc'">
      <template v-if="showType == 'app'">
        <a-form-item label="风格类型">
          <jnpf-select v-model:value="activeData.option.appStyleType" :options="appTodoStyleList" placeholder="请选择风格类型" showSearch />
        </a-form-item>
...
<script lang="ts" setup>
  ...
  function textChange() {
    props.activeData.option.defaultValue = '';
    if (props.activeData.dataType == 'static') props.activeData.option.defaultValue = 'KPI管理系统';
  }
  ...
  function getData() {
    return {
      ...
      data: props.showType == 'pc' ? props.activeData.option.defaultValue : props.activeData.option.appDefaultValue,
    };
  }
