<template>
  <a-collapse-panel>
    ...
    <template v-if="showType == 'pc'">
      <template v-if="activeData.option.styleType == 1">
        ...
        <a-form-item>
          ...
          <a-input-number v-model:value="activeData.option.noticeCount" :min="1" :max="99999" @change="renderKeyChange" />
        </a-form-item>
        <a-form-item label="字体大小">
      ...
      <template v-if="activeData.option.styleType != 1">
        ...
        <a-form-item>
          <template #label>显示条数<BasicHelp text="置空时，则显示全部数据" /></template>
          <a-input-number v-model:value="activeData.option.noticeCount" :min="1" :max="9999" @change="renderKeyChange" />
        </a-form-item>
      </template>
      <a-form-item label="背景色">
    ...
    <template v-else>
      ...
      <a-form-item>
        <template #label>显示条数<BasicHelp text="置空时，则显示全部数据" /></template>
        <a-input-number v-model:value="activeData.option.appCount" :min="1" :max="99999" @change="renderKeyChange" />
      </a-form-item>
    </template>
