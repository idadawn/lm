<template>
  <a-collapse-panel>
    ...
    <div class="options-list">
      <draggable v-model="activeData.children" :animation="300" group="selectItem" handle=".option-drag" itemKey="uuid">
        <template #item="{ element, index }">
          <div class="select-item">
            <div class="select-line-icon option-drag">
              <i class="icon-ym icon-ym-darg" />
            </div>
            <a-input class="!w-120px" v-model:value="element.title" placeholder="标签名称" />
            <JnpfIconPicker class="flex-1" v-model:value="element.icon" />
