<template>
  ...
  <Modal v-model:visible="visible" width="380px" :title="isEdit ? '编辑确认' : '删除确认'" centered>
...
<script lang="ts" setup>
    ...
    schemas: [
      {
        ...
        label: '类型',
        component: 'Select',
        componentProps: { placeholder: '请选择类型', allowClear: false },
      ...
      },
      {
        ...
        component: 'Select',
        defaultValue: '1',
        componentProps: { placeholder: '请选择紧急程度', options: urgentList, allowClear: false },
      },
      {
        field: 'title',
        label: '标题',
        ...
        rules: [{ required: true, trigger: 'change', message: '标题不能为空' }],
      },
      {
        field: 'content',
        label: '内容',
        component: 'Textarea',
        componentProps: { placeholder: '请输入内容', rows: 3 },
      },
      {
        field: 'allDay',
        label: '全天',
      ...
      },
      {
        ...
        colProps: { span: 16 },
        rules: [{ required: true, trigger: 'change', message: '开始时间不能为空' }],
      },
      {
        ...
        componentProps: { placeholder: '请选择开始时间', format: 'HH:mm', minuteStep: 5 },
        colProps: { span: 8 },
      },
      {
        ...
        component: 'Select',
        defaultValue: 60,
        componentProps: { placeholder: '请选择时长', options: durationList, allowClear: false, onChange: onDurationChange },
        rules: [{ required: true, trigger: 'change', message: '请选择时长', type: 'number' }],
      },
      {
        ifShow: ({ values }) => values.duration === -1 || !!values.allDay,
        field: 'endDay',
      ...
      },
      {
        ...
        label: '',
        component: 'TimePicker',
        componentProps: { placeholder: '请选择结束时间', format: 'HH:mm', minuteStep: 5 },
        colProps: { span: 8 },
      },
      {
        field: 'creatorUserId',
        ...
        componentProps: { placeholder: '请选择创建人', disabled: true },
      },
      {
        field: 'toUserIds',
        label: '参与人',
        component: 'UserSelect',
        componentProps: { placeholder: '请选择参与人', multiple: true },
      },
      {
        field: 'color',
        label: '标签颜色',
      ...
      },
      {
        ...
        component: 'Select',
        defaultValue: -2,
        componentProps: { placeholder: '请选择提醒时间', options: reminderTimeList, allowClear: false },
      },
      {
        ...
        component: 'Select',
        defaultValue: '1',
        componentProps: { placeholder: '请选择提醒方式', options: remindList, allowClear: false },
        rules: [{ required: true, trigger: 'change', message: '请选择提醒方式' }],
      },
      {
        ...
        component: 'Input',
        slot: 'send',
        rules: [{ required: true, trigger: 'blur', message: '发送配置不能为空' }],
      },
      {
        field: 'repetition',
        label: '重复提醒',
        component: 'Select',
        defaultValue: '1',
        componentProps: { placeholder: '请选择重复提醒', allowClear: false, options: repeatReminderList, onChange: onRepetitionChange },
      },
      {
        ...
        label: '结束重复',
        component: 'DatePicker',
        componentProps: { placeholder: '选择结束重复', format: 'YYYY-MM-DD' },
        rules: [{ required: true, trigger: 'change', message: '结束重复不能为空' }],
      },
    ],
  });
  ...
  async function initData() {
    ...
    updateSchema([{ field: 'type', componentProps: { options: list || [] } }]);
    if (list?.length && !state.dataForm.id) setFieldsValue({ type: list[0].id });
  }
