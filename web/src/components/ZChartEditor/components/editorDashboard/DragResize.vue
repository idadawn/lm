  ...
  export default {
    ...
    methods: {
      ...
      move(ev) {
        ...
        const delta = {
          x: parseInt((dimensionsBeforeMove.pointerX - pageX) / this.parentScaleX),
          y: parseInt((dimensionsBeforeMove.pointerY - pageY) / this.parentScaleY),
        };
    ...
    computed: {
      positionStyle() {
        return {
          ...
          left: this.left + 'px',
          zIndex: this.zIndex,
          border: this.style?.borderShow ? `1px solid ${this.style?.borderColor}` : 'none',
        };
      },
      ...
      vdrStick() {
        return stick => {
          const stickStyle = {
            width: `${parseInt(this.stickSize / this.parentScaleX)}px`,
            height: `${parseInt(this.stickSize / this.parentScaleY)}px`,
          };
          stickStyle[styleMapping.y[stick[0]]] = `${parseInt(this.stickSize / this.parentScaleX) / -2}px`;
          stickStyle[styleMapping.x[stick[1]]] = `${parseInt(this.stickSize / this.parentScaleX) / -2}px`;
          // stickStyle[styleMapping.y[stick[0]]] = `-1px`;
          // stickStyle[styleMapping.x[stick[1]]] = `-1px`;
          return stickStyle;
        };
      },
...
<style>
  ...
  .vdr {
    ...
    box-shadow: 0 1px 4px rgb(63 89 128 / 8%);
