...
export const useUserStore = defineStore({
  ...
  actions: {
    ...
    async getUserInfoAction(): Promise<GetUserInfoModel | null> {
      ...
      // changeLocale('zh_CN');
      let menus: BackMenu[] = [];
      if (userInfo.systemIds && userInfo.systemIds.length && routerList.length) {
        for (let i = 0; i < userInfo.systemIds.length; i++) {
          const e = userInfo.systemIds[i];
          if (e.currentSystem) {
            const list = routerList.filter(o => o.id === e.id);
            list.length ? (menus = list[0].children || []) : (menus = []);
            break;
          }
        }
      } else {
        menus = !routerList.length ? menuList : routerList[0].children ? routerList[0].children : [];
      }
      if (!menus.length) {
