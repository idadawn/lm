...
<script lang="ts" setup>
  ...
  const handleChange = (value: string) => {
    collectorConfigTemplateFun(value);
  };

  // 获取类型
  collectorDropDownFun();
  async function collectorDropDownFun() {
    const { data: res } = await collectorDropDown();
    type_options.value = res;
  }

  // 获取配置模板
  async function collectorConfigTemplateFun(data) {
    const { data: res } = await collectorConfigTemplate({ type: data });
  ...
  function getTableActions(record): ActionItem[] {
    return [
      ...
      {
        label: '标签',
        onClick: handleLabel.bind(null, record.id),
      },
      {
  ...
  const getDetail = async id => {
    let { data: res } = await collectorDetail({ id: id });
    if (res) {
      formState.id = res.id;
  ...
  const handleOk = async () => {
    formRef.value.validate().then(async () => {
      ...
      if (addEditTitle.value === '添加') {
        delete itemObj.id;
        let { success: res } = await collectorAdd(itemObj);
        if (res) {
        ...
        let { success: res } = await collectorUpdate(itemObj);
        if (res) {
          state.add_visible = false;
  ...
  function exportHandle() {
    systemDownloadCollectorConfig().then(res => {
      if (res) {
        // 下载导出json文件
        var a = document.createElement('a');
        a.download = '采集器配置.json';
  ...
  function handleState(record) {
    if (record.state === 0) {
      collectorDisable(record.id).then(res => {
        if (res) {
      ...
      collectorEnable(record.id).then(res => {
        if (res) {
          createMessage.success('操作成功');
          reload();
        }
  ...
  function handleLabel(id) {
    router.push('/collectorManagement/tag/' + id);
  }
</script>
