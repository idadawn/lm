...
<script lang="ts" setup>
  ...
  async function tagConfigTemplateFun() {
    const { data: res } = await tagConfigTemplateLogic({ id: state.parentId });
  ...
  const getDetail = async id => {
    let { data: res } = await tagDetailLogic({ id: state.parentId, tagId: id });
    if (res) {
  ...
  const handleOk = async () => {
    formRef.value.validate().then(async () => {
      ...
      if (addEditTitle.value === '添加') {
        ...
        let { success: res } = await tagAddLogic(itemObj);
        if (res) {
        ...
        let { success: res } = await tagUpdateLogic(itemObj);
        if (res) {
          state.add_visible = false;
  ...
  function handleDelete(id) {
    tagRemoveLogic(state.parentId, id).then(res => {
      if (res) {
