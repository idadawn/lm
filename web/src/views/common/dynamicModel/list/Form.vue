<template>
  <BasicPopup
    ...
    <template #insertToolbar>
      <a-space :size="10" v-if="config.id && config.showMoreBtn">
  ...
  <BasicModal
    ...
    <template #insertFooter>
      <a-space :size="10" v-if="config.id && config.showMoreBtn" class="float-left">
        <a-button :loading="state.prevBtnLoading" :disabled="getPrevDisabled" @click="handlePrev">上一条</a-button>
        <a-button :loading="state.nextBtnLoading" :disabled="getNextDisabled" @click="handleNext">下一条</a-button>
      </a-space>
    </template>
...
<script lang="ts" setup>
  ...
  const { title, config, formConf, key, loading, continueText, showContinueBtn } = toRefs(state);
  ...
  function init(data) {
    state.showContinueBtn = !data.formData;
