<template>
  <div class="page-content-wrapper short-link-wrapper short-link-wrapper-list">
            ...
            <template #expandedRowRender="{ record }" v-if="getChildTableStyle === 2 && childColumnList.length">
              <a-tabs size="small">
                <a-tab-pane :key="cIndex" :tab="child.label" :label="child.label" v-for="(child, cIndex) in childColumnList">
                  <a-table size="small" :data-source="record[child.prop]" :columns="child.children" :pagination="false"></a-table>
                </a-tab-pane>
              </a-tabs>
            </template>
...
<script lang="ts" setup>
  ...
  const [registerSearchForm, { setFieldsValue, submit: searchFormSubmit }] = useForm({
    baseColProps: { span: 6 },
