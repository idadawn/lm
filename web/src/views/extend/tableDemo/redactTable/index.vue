    ...
    <div class="page-content-wrapper-center">
      <div class="page-content-wrapper-content">
        <BasicTable @register="registerTable" @row-click="handleRowClick">
          <template #tableTitle>
            <a-button type="primary" preIcon="icon-ym icon-ym-btn-add" @click="addOrUpdateHandle()">新建项目</a-button>
          </template>
          <template #bodyCell="{ column, record, index }">
            ...
            <template v-else>
              <template v-if="state.tabClickId == record.id">
                ...
                <span v-else-if="column.key === 'registrant' || column.key === 'registerDate'">{{ record[column.key] }}</span>
              ...
              <template v-else>
                ...
                <span v-if="column.key === 'registerDate'">{{ record[column.key] ? dayjs(record[column.key]).format('YYYY-MM-DD HH:mm') : '' }}</span>
                <span v-if="column.key === 'projectType'">
                  {{
