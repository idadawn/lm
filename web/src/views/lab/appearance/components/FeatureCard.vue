  ...
  <div class="bg-white rounded-lg p-4 shadow-sm border border-gray-100 hover:shadow-md transition-shadow relative group">
    <div class="flex items-start justify-between mb-3">
      <div class="flex items-center gap-3">
         <div class="w-12 h-12 rounded bg-purple-50 flex items-center justify-center text-xl font-bold text-purple-600 border border-purple-100">
             {{ (feature.category || feature.name).charAt(0) }}
         </div>
         <div>
             <h3 class="font-bold text-lg text-gray-800">{{ feature.name || feature.category }}</h3>
             <div class="flex items-center gap-2 mt-1">
                <span class="px-2 py-0.5 bg-gray-100 text-gray-600 rounded text-xs border border-gray-200" v-if="feature.name && feature.category">
                    {{ feature.category }}
                </span>
                <span class="text-xs text-gray-500 truncate max-w-[150px]" :title="feature.description">{{ feature.description || '暂无描述' }}</span>
             </div>
         </div>
      </div>
