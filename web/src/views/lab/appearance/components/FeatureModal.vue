<template>
  <BasicModal v-bind="$attrs" @register="registerModal" :title="getTitle" @ok="handleSubmit" @cancel="handleCancel"
    :width="800" :height="550" :minHeight="400">
    <BasicForm @register="registerForm">
      <template #severitySlot="{ model, field }">
        <div style="display: flex; gap: 8px;">
          <a-select v-model:value="model[field]" placeholder="请选择特征等级" show-search allow-clear
            :options="getSeverityOptions" :field-names="{ label: 'fullName', value: 'id' }" style="flex: 1" />
          <a-button type="primary" @click="handleCreateLevel">
            <template #icon>
              <PlusOutlined />
            </template>
          </a-button>
      ...
      <template #keywordsInput="{ model, field }">
        <div class="keywords-tag-container">
          <a-input v-model:value="keywordInput" placeholder="输入关键词后按回车添加" @press-enter="handleAddKeyword"
            @blur="handleAddKeyword" allow-clear>
            <template #suffix>
              <span class="keyword-count">{{ model[field]?.length || 0 }} 个关键词</span>
            </template>
          </a-input>
          <div class="keywords-tags" v-if="model[field] && model[field].length > 0">
            <a-tag v-for="(keyword, index) in model[field]" :key="index" closable
              @close="handleRemoveKeyword(model, field, index)" color="blue">
              {{ keyword }}
            </a-tag>
          </div>
        </div>
    ...
    <!-- 新建等级弹窗 -->
    <LevelModal @register="registerLevelModal" @success="handleLevelCreated" />
  </BasicModal>
</template>
...
const [registerLevelModal, { openModal: openLevelModal }] = useModal();
...
const getSeverityOptions = computed(() => {
  return severityLevels.value.map(level => ({
    ...
    id: level.name, // 注意：这里的 id 用的是 name，因为后端需要 name 来匹配
  }));
});
...
// 等级创建成功回调
async function handleLevelCreated() {
  // 重新加载等级列表
  await loadSeverityLevels(true);
}
