<template>
  <div class="page-content-wrapper">
    <div class="page-content-wrapper-center">
        ...
        <div class="table-container">
          <BasicTable @register="registerTable">
            <template #form-toolbar>
              <div class="form-second-row">
                ...
                <div v-if="hasBtnP(PERM_FILL_COLOR)" class="color-fill-control">
                  ...
                  <a-button
                    size="small"
                    @click="clearSelectedColor"
                    :type="isClearMode ? 'primary' : 'default'"
                    :danger="isClearMode">
                    {{ isClearMode ? '清除模式' : '清除' }}
                  </a-button>
                  <a-button size="small" type="primary" @click="saveColorsBatch" :loading="savingColors"
                    >保存配置</a-button
                  >
                </div>
              </div>
            ...
            <template #tableTitle>
              ...
              <a-space>
                <a-upload :before-upload="handleQuickImport" :show-upload-list="false" accept=".xlsx,.xls">
                  <a-button> <UploadOutlined /> 磁性数据导入 </a-button>
                </a-upload>
                <a-button
                  v-if="hasBtnP(PERM_CALC)"
                  type="primary"
            ...
            <template #bodyCell="{ column, record, text }">
              ...
              <!-- 贴标列 -->
              <LabelingCell
                v-else-if="column.key === 'labeling'"
                :record="record"
                :text="text"
                :cell-class="getCellClass(record.id, column.key)"
              ...
              <!-- 日期字符串列（检测日期、生产日期等） -->
              <DateCell
                v-else-if="column.key === 'detectionDateStr' || column.key === 'prodDateStr'"
                :record="record"
                :text="text"
                :cell-class="getCellClass(record.id, column.key)"
                @click="handleCellColor(record.id, column.key, $event)" />
              ...
              <!-- 性能数据列 -->
              <PerfCell
                v-else-if="column.key?.startsWith('perf')"
                :record="record"
                :column="column"
                :cell-class="getCellClass(record.id, column.key)"
                :has-permission="hasBtnP(PERM_MAGNETIC)"
                :get-field-precision="getFieldPrecision"
                @click="handleCellColor(record.id, column.key, $event)"
                @save="val => handlePerfSave(record, column.key, val)" />

              <!-- 外观特性列 -->
              <FeatureCell
                v-else-if="column.key === 'appearanceFeatureList'"
                :record="record"
                :column="column"
                :cell-class="getCellClass(record.id, column.key)"
                :has-permission="hasBtnP(PERM_APPEARANCE)"
                :get-matched-feature-labels="getMatchedFeatureLabels"
                @click="handleCellColor(record.id, column.key, $event)"
                @dblclick="handleOpenFeatureDialog(record)" />

              <!-- 可编辑的测量值（中Si、中B、花纹、断头数、单卷重量等） -->
              <EditableMeasurementCell
                v-else-if="isEditableMeasurement(column.key)"
                :record="record"
                :column="column"
                :cell-class="getCellClass(record.id, column.key)"
                :has-permission="hasBtnP(PERM_APPEARANCE)"
              ...
              <template v-else-if="column.key === 'calcStatus'">
                <div class="cell-content">
                  <a-tooltip v-if="isCalcFailed(record)" :title="record.calcErrorMessage">
                    <a-tag
                      :color="getCalcStatusInfo(record.calcStatus).color"
              ...
              <template v-else-if="column.key === 'judgeStatus'">
                <div class="flex items-center justify-center cell-content">
                  <a-tag
                    :color="isNeedJudge(record) ? 'orange' : 'success'"
              ...
              <!-- 数值列 - 负数红色显示 -->
              <NumericCell
                v-else-if="isNumericColumn(column.key)"
                :record="record"
                :column="column"
                :text="text"
                :cell-class="getCellClass(record.id, column.key)"
                :format-numeric-value="formatNumericValue"
                @click="handleCellColor(record.id, column.key, $event)" />

              <!-- 动态检测列 -->
              <NumericCell
                v-else-if="column.key?.startsWith('detection')"
                :record="record"
                :column="column"
                :text="text"
                :cell-class="getCellClass(record.id, column.key)"
              ...
              <!-- 动态带厚列 -->
              <NumericCell
                v-else-if="column.key?.startsWith('thickness') && column.key !== 'thicknessRange'"
                :record="record"
                :column="column"
                :text="text"
                :cell-class="getCellClass(record.id, column.key)"
              ...
              <!-- 其他列 -->
              <DefaultCellWithNumeric
                v-else
                :record="record"
                :column="column"
                :cell-class="getCellClass(record.id, column.key)"
                :is-numeric-string="isNumericString"
      ...
      <CalcProgressBar :active-list="activeProgressList" :completed-list="recentCompletedList" />
      ...
      <a-modal
        ...
        <a-form layout="vertical" :style="{ marginBottom: 0 }">
          ...
          <a-form-item v-if="exportDateRange && exportDateRange.length === 2" style="margin-bottom: 0">
            <a-alert
              type="info"
              :message="`将导出 ${formatExportDate(exportDateRange[0])} 至 ${formatExportDate(
                exportDateRange[1],
              )} 的数据`"
              show-icon />
          </a-form-item>
            ...
            <div class="trace-overview">
              ...
              <div class="trace-status-card">
                ...
                <div class="trace-status-value">
                  <a-tag :color="getTraceStatusColor(calcTraceData.calcStatusText, calcTraceData.calcErrorMessage)">{{
                    getTraceStatusLabel(calcTraceData.calcStatusText, calcTraceData.calcErrorMessage)
                  }}</a-tag>
                </div>
              </div>
              <div class="trace-status-card">
                ...
                <div class="trace-status-value">
                  <a-tag :color="getTraceStatusColor(calcTraceData.judgeStatusText, calcTraceData.judgeErrorMessage)">{{
                    getTraceStatusLabel(calcTraceData.judgeStatusText, calcTraceData.judgeErrorMessage)
                  }}</a-tag>
                </div>
              </div>
            </div>
                    ...
                    <div
                      ...
                      <div class="trace-step-header">
                        <div class="trace-step-title"
                          >步骤 {{ step.order }} ·
                          {{ step.formulaName || step.displayName || step.columnName || '-' }}</div
                        >
                        <a-tag :color="step.success ? 'success' : 'error'">{{ step.success ? '成功' : '失败' }}</a-tag>
                      </div>
                      <div class="trace-grid">
                        <div
                          ><span class="trace-key">公式名称</span
                          ><span class="trace-value">{{ step.formulaName || '-' }}</span></div
                        >
                        <div
                          ><span class="trace-key">对应列名</span
                          ><span class="trace-value">{{ step.displayName || step.columnName || '-' }}</span></div
                        >
                        <div
                          ><span class="trace-key">公式</span
                          ><span class="trace-value trace-formula">{{
                            step.formula || (step.isDefaultValue ? '使用默认值' : '-')
                          }}</span></div
                        >
                        <div
                          ><span class="trace-key">精度</span
                          ><span class="trace-value">{{ step.precision ?? '-' }}</span></div
                        >
                        <div
                          ><span class="trace-key">是否失败</span
                          ><span class="trace-value">{{ step.success ? '否' : '是' }}</span></div
                        >
                        <div
                          ><span class="trace-key">计算结果</span
                          ><span class="trace-value">{{ step.resultValue || step.rawResult || '-' }}</span></div
                        >
                      </div>
                      <div class="trace-subtitle">参与计算的数值</div>
                      <div v-if="step.variableValues?.length" class="trace-chip-list">
                        <span v-for="item in step.variableValues" :key="`${step.order}-${item.name}`" class="trace-chip"
                          >{{ item.name }} = {{ item.value || '空' }}</span
                        >
                      </div>
                  ...
                  <div v-else class="trace-judge-layout">
                    <div class="trace-judge-nav">
                      <button
                        ...
                        <div class="trace-judge-nav-top">
                          ...
                          <span class="trace-judge-nav-status" :class="`trace-judge-nav-status--${group.status}`">
                            {{ group.statusText }}
                          </span>
                        </div>
                    ...
                    <div v-if="activeJudgeProject" class="trace-project-card">
                      <div class="trace-project-header">
                        ...
                        <a-tag
                          :color="
                            activeJudgeProject.hitStep ? 'success' : activeJudgeProject.hasFailure ? 'error' : 'default'
                          ">
                          {{
                            activeJudgeProject.hitStep
                              ? `命中 ${
                                  activeJudgeProject.hitStep.stepName || activeJudgeProject.hitStep.resultValue || '-'
                                }`
                              : activeJudgeProject.hasFailure
                              ? '执行异常'
                              : '未命中'
                          }}
                        </a-tag>
                      ...
                      <div class="trace-level-list">
                        <div
                          v-for="step in activeJudgeProject.steps"
                          :key="`judge-${activeJudgeProject.key}-${step.order}-${step.stepName}`"
                          class="trace-level-row">
                          <div class="trace-level-main">
                            ...
                            <div class="trace-condition trace-condition--multiline">{{
                              formatJudgeConditionText(step.conditionText)
                            }}</div>
                            <div v-if="step.failureReason" class="trace-error-box"
                              >失败原因：{{ step.failureReason }}</div
                            >
                          </div>
...
<script lang="ts" setup>
  ...
  import { saveIntermediateDataColors, getIntermediateDataColors } from '../../../api/lab/intermediateDataColor';
  ...
  import type {
    IntermediateDataExecutionTrace,
    IntermediateDataCalcStepItem,
    IntermediateDataJudgeStepItem,
  } from '/@/api/lab/model/intermediateDataCalcLogModel';
  ...
  function getMatchedFeatureLabels(record: any): Array<{ id: string; label: string }> {
    ...
    if (record.appearanceFeatureDetails && record.appearanceFeatureDetails.length > 0) {
      return record.appearanceFeatureDetails.map((detail: any) => ({
        ...
        label: [detail.categoryName, detail.severityLevelName].filter(Boolean).join(' / '),
      }));
    }
  ...
  const allColumns = computed(() => {
    ...
    const columns: BasicColumn[] = [
      ...
      {
        title: '检验日期',
        dataIndex: 'detectionDateStr',
        key: 'detectionDateStr',
        width: 80,
        fixed: 'left',
        align: 'center',
      },
      ...
      { title: '一米带重(g)', dataIndex: 'oneMeterWeight', key: 'oneMeterWeight', width: 80, align: 'center' },
      { title: '带宽 (mm)', dataIndex: 'width', key: 'width', width: 80, align: 'center' },
      // --- 动态列：带厚 ---
      {
        title: '带厚',
        key: 'thickness_group',
      ...
      // --- 规格与物理特性 ---
      { title: '带厚极差', dataIndex: 'thicknessDiff', key: 'thicknessDiff', width: 70, align: 'center' },
      { title: '密度 (g/cm³)', dataIndex: 'density', key: 'density', width: 70, align: 'center' },
      { title: '叠片系数', dataIndex: 'laminationFactor', key: 'laminationFactor', width: 70, align: 'center' },

      // --- 外观缺陷区 ---
      {
        title: '外观特性',
        dataIndex: 'appearanceFeatureList',
        key: 'appearanceFeatureList',
        width: 220,
        align: 'center',
      },

      { title: '断头数(个)', dataIndex: 'breakCount', key: 'breakCount', width: 70, align: 'center' },
      { title: '单卷重量(kg)', dataIndex: 'singleCoilWeight', key: 'singleCoilWeight', width: 80, align: 'center' },
      { title: '外观检验员', dataIndex: 'appearEditorName', key: 'appearEditorName', width: 80, align: 'center' },
      // --- 汇总判定 ---
      { title: '平均厚度', dataIndex: 'avgThickness', key: 'avgThickness', width: 80, align: 'center' },
      ...
      { title: '四米带重(g)', dataIndex: 'coilWeight', key: 'coilWeight', width: 80, align: 'center' },
      ...
      { title: '最大厚度', dataIndex: 'maxThicknessRaw', key: 'maxThicknessRaw', width: 80, align: 'center' },
      { title: '最大平均厚度', dataIndex: 'maxAvgThickness', key: 'maxAvgThickness', width: 100, align: 'center' },
      // --- 录入信息 ---
      { title: '录入人', dataIndex: 'creatorUserName', key: 'creatorUserName', width: 80, align: 'center' },
      { title: '带型', dataIndex: 'stripType', key: 'stripType', width: 80, align: 'center' },
  ...
  const calcCalculationSteps = computed<IntermediateDataCalcStepItem[]>(
    () => calcTraceData.value?.calculationSteps || [],
  );
  ...
  const activeJudgeProject = computed<JudgeProjectGroup | undefined>(() => {
    ...
    return (
      judgeProjectGroups.value.find(item => item.key === activeJudgeProjectKey.value) || judgeProjectGroups.value[0]
    );
  });
  ...
  async function handleBatchRecalculate() {
    ...
    // 有勾选时只计算勾选的，没勾选时计算全部
    const ids = selectedIdSet.size > 0 ? Array.from(selectedIdSet) : (getDataSource() as Recordable[]).map(r => r.id);
