<template>
  <BasicModal v-bind="$attrs" @register="registerModal" :title="getTitle" @ok="handleSubmit(0)" :showContinueBtn="!id"
    @continue="handleSubmit(1)" @cancel="handleCancel" :width="1200" wrapClassName="product-spec-modal"
    ...
    <div class="product-spec-form-content">
      <Tabs default-active-key="1">
        <TabPane key="1" tab="基本信息">
          ...
          <!-- 版本说明提示（仅编辑时显示） -->
          <div v-if="id && createNewVersion"
            style="margin: -16px 0 16px 0; padding: 8px 16px; background: #e6f7ff; border-left: 3px solid #1890ff; border-radius: 2px;">
            <p style="margin: 0; color: #666; font-size: 12px;">
              提示：创建新版本后，历史数据将继续使用旧版本，新数据将使用新版本。
            </p>
          </div>
          <ExtendedAttributesForm v-model="extendedAttributes" :showAddAndClear="!!id"
