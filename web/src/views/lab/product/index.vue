  ...
  <div class="p-4 h-full flex flex-col gap-4">
    ...
    <div class="bg-white p-4 rounded-lg shadow-sm flex justify-between items-center">
      ...
      <div class="flex gap-2">
        ...
        <a-button type="primary" :icon="h(PlusOutlined)" @click="addOrUpdateHandle()"
    ...
    <div class="flex-1 overflow-auto bg-gray-50 p-4 rounded-lg">
      <a-spin :spinning="loading">
        ...
        <draggable v-else v-model="dataList" item-key="id" handle=".drag-handle" :animation="200"
          ghost-class="ghost-card" chosen-class="chosen-card" @end="handleDragEnd"
            ...
            <div
              ...
              <div class="flex items-start justify-between mb-3">
                ...
                <div class="flex flex-col items-end gap-1 relative" style="min-width: 60px;">
                  ...
                  <div class="hidden group-hover:flex gap-1 flex-col" @mousedown.stop @click.stop>
                    <a-button type="link" size="small" @click.stop="addOrUpdateHandle(record.id)" @mousedown.stop
                      style="padding: 0 4px;">编辑</a-button>
                    <a-button type="link" size="small" danger @click.stop="handleDeleteClick(record)" @mousedown.stop
                      style="padding: 0 4px;">删除</a-button>
                  </div>
                </div>
              </div>
              ...
              <div class="space-y-2 mt-3 pt-3 border-t border-gray-100">
                ...
                <template v-if="record.attributes && record.attributes.length > 0">
                  <div v-for="attr in record.attributes.slice(0, 3)" :key="attr.attributeKey"
      ...
      <div class="pagination-wrapper mt-4" v-if="dataList.length > 0">
        <a-pagination v-model:current="pagination.current" v-model:page-size="pagination.pageSize"
          :total="pagination.total" :show-size-changer="true" :show-total="(total) => `共 ${total} 条`"
