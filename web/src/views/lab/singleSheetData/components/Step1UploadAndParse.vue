<template>
  <div class="step1-container">
    ...
    <div class="upload-section">
      <a-alert message="第一步：文件上传与解析"
        description="请上传Excel文件，系统将自动解析B列(原始炉号)、H列(Ps铁损)、I列(Ss激磁功率)、F列(Hc)、P列(检测时间)，以导入模板配置为准。"
...
async function handleFile(file: File) {
  ...
  try {
    ...
    if (!currentSessionId) {
      // 创建导入会话（上传文件并保存到后端）
      const result = await createSingleSheetImportSession({
        fileName: file.name,
        fileData: base64,
      });
    ...
    // 上传并解析文件
    const parseResult = await uploadAndParseSingleSheetData(currentSessionId, {
      fileName: file.name,
      fileData: base64,
    });
