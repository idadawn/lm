...
import { getSingleSheetImportReview, completeSingleSheetImport } from '/@/api/lab/singleSheetData';
...
async function loadReviewData() {
  ...
  try {
    // 获取核对数据
    const review = await getSingleSheetImportReview(props.importSessionId);
    // 调试日志
...
async function handleStartImport() {
  ...
  try {
    ...
    await completeSingleSheetImport(props.importSessionId);
    ...
    message.success('单片性能数据已写入，更新和判定正在后台处理');
    emit('complete');
  } catch (error: any) {
