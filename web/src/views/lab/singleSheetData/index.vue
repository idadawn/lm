    ...
    <div class="page-content-wrapper-center">
      <div class="page-content-wrapper-content">
        <a-tabs v-model:activeKey="activeTab" type="card">
          <!-- 单片性能数据页签 -->
          <a-tab-pane key="data" tab="单片性能数据">
      ...
      <SingleSheetImportQuickModal @register="registerQuickModal" @reload="handleImportSuccess" />
    </div>
  </div>
...
import { getSingleSheetRawDataList, deleteSingleSheetRawData } from '/@/api/lab/singleSheetData';
...
import SingleSheetImportQuickModal from './SingleSheetImportQuickModal.vue';
import { createSingleSheetImportSession, uploadAndParseSingleSheetData } from '/@/api/lab/singleSheetData';


defineOptions({ name: 'singleSheet' });
...
async function handleDelete(record: Recordable) {
  await deleteSingleSheetRawData(record.id);
...
const [registerTable, { reload }] = useTable({
  ...
  afterFetch: (data) => {
    console.log('SingleSheetData afterFetch:', data);
...
async function handleQuickImport(file: File) {
  ...
  try {
    ...
    // 2. 创建会话并保存文件
    const sessionId = await createSingleSheetImportSession({
      fileName: file.name,
      fileData: fileData,
    });
    ...
    // 3. 执行解析
    const parseResult = await uploadAndParseSingleSheetData(sessionId, {
      fileName: file.name,
      fileData: fileData,
    });
