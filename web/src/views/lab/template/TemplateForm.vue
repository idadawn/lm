        ...
        <a-table :dataSource="fieldMappings" :columns="columns" :pagination="false" size="small" rowKey="key">
          <template #bodyCell="{ column, record, index }">
            ...
            <template v-if="column.key === 'excelColumnNames'">
              <a-select
                v-model:value="record.excelColumnIndex"
                style="width: 100%"
                placeholder="选择列"
                :options="getOptionsForRecord(record)"
                allowClear
...
// 表格列定义
const columns = [
  { title: '字段名称', key: 'field', width: '20%' },
  { title: 'Excel列名 (选择)', key: 'excelColumnNames', width: '30%' },
  { title: '数据类型', key: 'dataType', width: '15%' },
  { title: '单位', key: 'unitId', width: '25%' }, // Adjusted width
  { title: '必填', key: 'required', width: '10%', align: 'center' },
  // 移除操作列
];
