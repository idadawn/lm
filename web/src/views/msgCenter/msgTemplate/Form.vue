<template>
  <BasicPopup v-bind="$attrs" @register="registerPopup" showOkBtn :title="title" @ok="handleSubmit" destroy-on-close>
      ...
      <a-row :gutter="20">
        <a-col :span="12">
          <a-form-item label="名称" name="fullName">
            <a-input v-model:value="dataForm.fullName" placeholder="请输入名称" allowClear />
          </a-form-item>
        </a-col>
        <a-col :span="12">
          <a-form-item label="编码" name="enCode">
            <a-input v-model:value="dataForm.enCode" placeholder="请输入编码" allowClear :maxlength="50" />
          </a-form-item>
        </a-col>
        <a-col :span="12">
        ...
        <a-col :span="24">
          <a-form-item label="说明" name="description">
            <a-textarea v-model:value="dataForm.description" placeholder="请输入说明" type="textarea" :rows="3" />
          </a-form-item>
        </a-col>
        <a-col :span="24">
          <div class="parameter-box">
            ...
            <div class="right-pane" v-if="dataForm.messageType != 3 && dataForm.messageType != 7">
              <a-form-item name="title">
                ...
                <a-input v-model:value="dataForm.title" placeholder="请输入消息标题" allowClear />
              </a-form-item>
              ...
              <a-form-item name="content" v-else-if="dataForm.messageType != 1">
                <template #label>消息内容<BasicHelp :text="getTipLabel()" /></template>
                <a-textarea v-model:value="dataForm.content" placeholder="请输入消息内容" :rows="16" />
              </a-form-item>
            </div>
              ...
              <a-row :gutter="20">
                <a-col :span="12">
                  <a-form-item name="templateCode">
                    ...
                    <a-input v-model:value="dataForm.templateCode" placeholder="请输入模版编号" />
                  </a-form-item>
                </a-col>
                <a-col :span="12" offset="12"></a-col>
                <a-col :span="12" v-if="dataForm.messageType == 7">
                  <a-form-item label="跳转方式" name="wxSkip">
                    <jnpf-select v-model:value="dataForm.wxSkip" placeholder="请选择跳转方式" :options="wxSkipList" />
                  </a-form-item>
                </a-col>
                <a-col :span="12" v-if="dataForm.messageType == 7 && dataForm.wxSkip == 1">
                  <a-form-item name="xcxAppId" :labelCol="{ style: { width: '120px' } }">
                    ...
                    <a-input v-model:value="dataForm.xcxAppId" placeholder="请输入关联小程序ID" allowClear />
                  </a-form-item>
                </a-col>
              </a-row>
              <div class="msg-pane">
    ...
    <BasicModal @register="registerModal" :title="isEdit ? '编辑参数' : '添加参数'" showOkBtn @ok="handleFieldSubmit" destroyOnClose>
...
<script lang="ts" setup>
  ...
  import { getInfo, create, update, getMsgTypeList } from '/@/api/msgCenter/msgTemplate';
  ...
  const schemas: FormSchema[] = [
    ...
    },
    {
      ...
      label: '参数说明',
      component: 'Input',
      componentProps: { placeholder: '请输入参数说明' },
  ...
  function init(data) {
    ...
    state.title = state.dataForm.id ? '编辑消息模板' : '新建消息模板';
    state.parameterList = [];
