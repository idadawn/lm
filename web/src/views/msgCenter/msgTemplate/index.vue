...
<script lang="ts" setup>
  ...
  import { getMsgTemplateList, delMsgTemplate, copy, getMsgTypeList } from '/@/api/msgCenter/msgTemplate';
  ...
  async function init() {
    const data: any[] = await baseStore.getMsgTypeData();
    data.map(o => (o.id = o.enCode));
    state.messageTypeList = data;
    let res = await getMsgTypeList(4);
    res.data.map(o => (o.id = o.enCode));
    getForm().updateSchema([
      { field: 'messageType', componentProps: { options: data } },
      { field: 'messageSource', componentProps: { options: res.data } },
    ]);
  }
