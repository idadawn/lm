...
<script lang="ts" setup>
  ...
  const schemas: FormSchema[] = [
    {
      ...
      label: '消息类型',
      component: 'Select',
      componentProps: { placeholder: '选择消息类型', onChange: onMessageTypeChange },
    ...
    },
    {
      ...
      component: 'Select',
      slot: 'templateId',
      rules: [{ required: true, trigger: 'change', message: '模板名称不能为空' }],
    },
    {
      field: 'templateCode',
      label: '模板编码',
      component: 'Input',
      componentProps: { placeholder: '模板编码', disabled: true },
    },
    {
      ifShow: ({ values }) => values.messageType != 1,
      field: 'accountConfigId',
      label: '账号名称',
      component: 'Select',
      slot: 'accountConfigId',
      rules: [{ required: true, trigger: 'change', message: '账号名称不能为空' }],
    },
    {
      ifShow: ({ values }) => values.messageType != 1,
      field: 'accountCode',
  ...
  async function initData() {
    let messageTypeList = await baseStore.getMsgTypeData();
