...
<script lang="ts" setup>
  ...
  const fieldColumns: BasicColumn[] = [
    ...
    { title: '条件符号', dataIndex: 'conditionSymbolName', width: 80 },
    {
      ...
      width: 100,
      customRender: ({ record }) => {
        if (record.conditionText == 'text') return '任意文本';
        if (record.conditionText == '@userId') return '当前用户';
        if (record.conditionText == '@organizeId') return '当前组织';
        if (record.conditionText == '@organizationAndSuborganization') return '当前组织及子组织';
        if (record.conditionText == '@userAraSubordinates') return '当前用户及下属';
        if (record.conditionText == '@branchManageOrganize') return '当前分管组织';
        if (record.conditionText == '@branchManageOrganizeAndSub') return '当前分管组织及子组织';
      },
    },
  ];
