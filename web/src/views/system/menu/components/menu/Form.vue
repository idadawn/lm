  ...
  <BasicModal v-bind="$attrs" @register="registerModal" :title="getTitle" @ok="handleSubmit" destroy-on-close>
    <BasicForm @register="registerForm">
      ...
      <template #urlAddress="{ model, field }">
        <a-input v-model:value="model[field]" placeholder="请输入地址">
...
<script lang="ts" setup>
  ...
  const validateUrl = (_rule, value) => {
    if (getFieldsValue()?.type == 7 && !isUrl(value)) return Promise.reject('请输入正确的链接地址');
  ...
  const schemas: FormSchema[] = [
    {
      ...
      componentProps: { placeholder: '请选择上级菜单', onChange: onParentIdChange },
    ...
    },
    {
      ...
      rules: [{ required: true, message: '菜单图标不能为空', trigger: 'change' }],
    },
    {
      field: 'type',
      label: '类型',
      ...
      rules: [{ required: true, trigger: 'change', message: '请选择菜单类型', type: 'number' }],
    },
    {
    ...
    },
    {
      ...
      component: 'TreeSelect',
      slot: 'moduleId',
      rules: [{ required: true, message: '关联不能为空', trigger: 'change' }],
    },
    {
      field: 'sortCode',
      label: '排序',
  ...
  const getTitle = computed(() => (!state.id ? '新建菜单' : '编辑菜单'));
  ...
  async function handleSubmit() {
    ...
    if (state.category === 'Web') {
      ...
      if (values.type == 5) values.urlAddress = menuEnCode;
    }
    const query = {
      ...state.dataForm,
      ...values,
