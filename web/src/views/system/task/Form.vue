      ...
      <a-col span="14" offset="5">
        <BasicForm @register="registerForm">
          <template #taskCron="{ model, field }">
            <JnpfCron v-model:value="model[field]" />
          </template>
...
<script lang="ts" setup>
  ...
  const getTitle = computed(() => (!unref(id) ? '新建任务' : '编辑任务'));
  ...
  const schemas: FormSchema[] = [
    {
      ...
      label: '任务名称',
      component: 'Input',
      componentProps: { placeholder: '任务名称' },
    ...
    },
    {
      ...
      label: '任务编码',
      component: 'Input',
      componentProps: { placeholder: '任务编码' },
      rules: [
        { required: true, trigger: 'blur', message: '任务编码不能为空' },
        { validator: formValidate('enCode', '任务编码只能输入英文、数字和小数点且小数点不能放在首尾'), trigger: 'blur' },
      ],
    },
    {
      ...
      component: 'Input',
      slot: 'taskCron',
      rules: [{ required: true, trigger: 'blur', message: 'Cron表达式不能为空' }],
    },
    {
      ...
      rules: [
        { required: true, trigger: 'change', message: '任务开始时间不能为空' },
    ...
    },
    {
      ...
      rules: [{ required: true, trigger: 'change', message: '方法选择不能为空' }],
    },
    {
      field: 'enabledMark',
      label: '任务状态',
