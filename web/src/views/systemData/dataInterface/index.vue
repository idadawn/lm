      ...
      <div class="page-content-wrapper-content">
        <BasicTable @register="registerTable" :searchInfo="searchInfo" :tableSetting="tableSetting">
          <template #tableTitle>
            ...
            <jnpf-upload-btn url="/api/system/DataInterface/Action/Import" accept=".bd" @on-success="reload"></jnpf-upload-btn>
          </template>
...
<script lang="ts" setup>
  ...
  const columns: BasicColumn[] = [
    ...
    { title: '编码', dataIndex: 'enCode' },
    {
      ...
      width: 100,
      customRender: ({ record }) => {
        return record.dataType == 1 ? 'SQL操作' : record.dataType == 2 ? '静态数据' : 'API操作';
      },
    },
  ...
  async function initData(isInit = false) {
    ...
    nextTick(() => {
      if (isInit) searchInfo.categoryId = unref(treeData)[0].id;
      const leftTree = unref(leftTreeRef);
      leftTree?.setSelectedKeys([searchInfo.categoryId]);
      if (isInit) searchInfo.categoryId && reload();
