<template>
  <BasicPopup v-bind="$attrs" @register="registerModal" :title="getTitle" showOkBtn @ok="handleSubmit">
    ...
    <a-table :data-source="fieldList" :columns="columns" size="small" :pagination="false" rowKey="index" class="drag-table">
...
<script lang="ts" setup>
    ...
    schemas: [
      {
        ...
        label: '表名称',
        component: 'Input',
        componentProps: { placeholder: '表名称', maxlength: 50 },
        rules: [
          { required: true, trigger: 'blur', message: '表名称不能为空' },
      ...
      },
      {
        ...
        label: '表说明',
        component: 'Input',
        componentProps: { placeholder: '表说明', maxlength: 50 },
        rules: [{ required: true, trigger: 'blur', message: '表说明不能为空' }],
      },
    ],
  });
  const columns: any[] = [
  ...
  const getTitle = computed(() => (!unref(dataForm.value.table) ? '新建表名' : '编辑表名'));
  ...
  function init(data) {
    ...
    if (dataForm.value.table) {
      getInfo(unref(linkId), dataForm.value.table).then(res => {
        ...
        let tableFieldList = res.data.tableFieldList.map(o => ({ ...o, index: buildBitUUID(), primaryKey: !!o.primaryKey, allowNull: !!o.allowNull }));
  ...
  async function handleSubmit() {
    ...
    const query = { tableFieldList: tableFieldList.map(o => ({ ...o, primaryKey: o.primaryKey ? 1 : 0, allowNull: o.allowNull ? 1 : 0 })), tableInfo };
