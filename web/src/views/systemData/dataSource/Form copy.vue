...
<script lang="ts" setup>
  ...
  const schemas: FormSchema[] = [
    {
      ...
      rules: [{ required: true, trigger: 'change', message: '连接驱动不能为空' }],
    },
    {
      field: 'fullName',
      label: '连接名称',
      component: 'Input',
      componentProps: { placeholder: '连接名称', maxlength: 50 },
      rules: [{ required: true, trigger: 'blur', message: '连接名称不能为空' }],
    },
    {
      field: 'host',
      label: '主机地址',
      component: 'Input',
      componentProps: { placeholder: '主机地址' },
      rules: [{ required: true, trigger: 'blur', message: '主机地址不能为空' }],
    },
    {
      field: 'port',
      label: '端口',
      component: 'Input',
      componentProps: { placeholder: '端口' },
      rules: [
        { required: true, trigger: 'blur', message: '端口不能为空' },
        {
    ...
    },
    {
      ...
      label: '用户',
      component: 'Input',
      componentProps: { placeholder: '用户' },
      rules: [{ required: true, trigger: 'blur', message: '用户不能为空' }],
    },
    {
      field: 'password',
      label: '密码',
    ...
    },
    {
      ifShow: ({ values }) =>
        ...
        values.dbType === 'PostgreSQL' ||
        values.dbType === 'KingbaseES',
      field: 'serviceName',
      label: '库名',
      component: 'Input',
      slot: 'serviceName',
      rules: [{ required: true, trigger: 'blur', message: '库名不能为空' }],
    },
    {
    ...
    },
    {
      ...
      label: '服务名',
      component: 'Input',
      componentProps: { placeholder: '服务名' },
    },
    {
      field: 'sortCode',
      label: '排序',
  ...
  const getTitle = computed(() => (!unref(id) ? '新建连接' : '编辑连接'));
  const emit = defineEmits(['register', 'reload', 'dbTypeModal']);
  const { createMessage } = useMessage();
  const [registerForm, { setFieldsValue, validate, clearValidate, resetFields, updateSchema }] = useForm({
    labelWidth: 80,
    schemas: schemas,
  });
