  ...
  <div class="page-content-wrapper">
    <div class="page-content-wrapper-center">
      <div class="page-content-wrapper-search-box">
        <BasicForm @register="registerSearchForm" @submit="handleSubmit" @reset="handleReset"></BasicForm>
      </div>
      <div class="page-content-wrapper-content bg-white">
        <a-tabs v-model:activeKey="activeKey" type="card" class="page-content-wrapper-tabs" destroyInactiveTabPane @tabClick="onTabClick">
          <a-tab-pane key="0" tab="委托发起">
            <BasicTable @register="registerLaunchTable" :searchInfo="getSearchInfo">
              <template #tableTitle>
                <a-button type="primary" preIcon="icon-ym icon-ym-btn-add" @click="openAddModal()">新建流程</a-button>
              </template>
              <template #bodyCell="{ column, record }">
                <template v-if="column.key === 'status'">
                  ...
                  <a-tag v-else-if="record.status == 4">流程撤回</a-tag>
          ...
          <a-tab-pane key="1" tab="委托设置">
            <BasicTable
              ...
              <template #tableTitle>
                <a-button type="primary" preIcon="icon-ym icon-ym-btn-add" @click="addOrUpdateHandle()">新建委托</a-button>
              </template>
              <template #bodyCell="{ column, record }">
                <template v-if="column.key === 'status'">
...
<script lang="ts" setup>
  ...
  const delegateTableColumns: BasicColumn[] = [
    ...
    { title: '开始时间', dataIndex: 'startTime', width: 150, format: 'date|YYYY-MM-DD HH:mm' },
    { title: '结束时间', dataIndex: 'endTime', width: 150, format: 'date|YYYY-MM-DD HH:mm' },
