...
<script lang="ts" setup>
  import { reactive, toRefs, onMounted, ref, unref, nextTick } from 'vue';
    ...
    const loop = (list, parent?) => {
      for (let i = 0; i < list.length; i++) {
        ...
        if (item.__vModel__) {
          ...
          isDisabled = item.disabled || state.config.disabled ? true : isDisabled;
          noShow = item.__config__.noShow ? item.__config__.noShow : noShow;
          if (state.config.readonly) isDisabled = true;
